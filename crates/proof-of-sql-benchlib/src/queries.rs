...
impl BaseEntry for Arithmetic {
    ...
    fn params(&self) -> Vec<LiteralValue> {
        vec![LiteralValue::BigInt(0), LiteralValue::BigInt(1)]
    }
}
...
impl BaseEntry for ComplexCondition {
    ...
    fn params(&self) -> Vec<LiteralValue> {
        vec![LiteralValue::Int(1), LiteralValue::VarChar("a".to_string())]
    }
}
...
impl BaseEntry for Coin {
    ...
    fn sql(&self) -> &'static str {
        "SELECT COUNT(1) AS num_transactions, SUM(amount) AS total_balance, SUM(amount * 2) AS total_balance_squared FROM bench_table;"
    }
...
impl BaseEntry for UnionAll {
    ...
    fn params(&self) -> Vec<LiteralValue> {
        vec![LiteralValue::Int128(10), LiteralValue::Int128(20)]
    }
}
