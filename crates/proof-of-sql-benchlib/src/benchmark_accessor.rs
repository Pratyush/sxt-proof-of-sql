...
impl<'a, C: Commitment> BenchmarkAccessor<'a, C> {
    ...
    #[must_use]
    pub fn table_schema(&self, table_ref: &TableRef) -> Option<Vec<(Ident, ColumnType)>> {
        self.table_schemas.get(table_ref).cloned()
    }
    ...
    pub fn table_columns(
        &self,
        table_ref: &TableRef,
    ) -> Option<Vec<(Ident, Column<'a, C::Scalar>)>> {
        let schema = self.table_schemas.get(table_ref)?;
        let columns = schema
            .iter()
            .map(|(ident, _)| {
                let col = *self.columns.get(&(table_ref.clone(), ident.clone()))?;
                Some((ident.clone(), col))
            })
